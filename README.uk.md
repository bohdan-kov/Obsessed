...
#### Огляд аналітичного дашборду
![Мобільний дашборд](./screenshots/01-mobile-dashboard.png)
*Головна аналітична панель з загальною кількістю тренувань, піднятим об'ємом, днями відпочинку та поточною серією. Чистий дизайн на основі карток показує ключові метрики з першого погляду — ідеально для відстеження прогресу між підходами. Включає вибір періоду (Цього місяця) з індикаторами трендів та мотиваційними інсайтами (Сильна послідовність, Об'єм зростає).*

#### Швидкий вибір вправи
![Швидке логування](./screenshots/02-mobile-quick-log.png)
...
![Активне тренування](./screenshots/03-mobile-active-workout.png)
...
#### Мобільна навігація
![Мобільна навігація](./screenshots/04-mobile-navigation.png)
...
<details>
<summary>Клікніть щоб розгорнути планшетні скріншоти</summary>

#### Огляд дашборду
![Планшетний дашборд](./screenshots/05-tablet-dashboard.png)
*Головна панель на iPad у портретній орієнтації з картками статистики, графіками трендів та таблицею вправ. Портретна орієнтація забезпечує природний досвід прокрутки для перегляду всіх метрик та останніх тренувань — ідеально для аналізу продуктивності між підходами з більшим та зручнішим дисплеєм, ніж телефон.*

#### Аналітичний дашборд - відстеження об'єму
![Теплова карта аналітики планшет](./screenshots/06-tablet-analytics-heatmap.png)
*365-денна теплова карта об'єму та графіки прогресивного перевантаження на iPad у портретній орієнтації. Вертикальний планшетний формат ідеально підходить для перегляду розширеної історії тренувань під час відстеження тренувань у залі, показуючи тижні даних з чіткими кольоровими рівнями інтенсивності.*

#### Порівняння прогресу вправ
![Таблиця прогресу вправ планшет](./screenshots/07-tablet-exercise-progress.png)
*Таблиця прогресу вправ на iPad у портретній орієнтації, що відображає 10+ вправ з приблизним 1ПМ, індикаторами трендів та міні-графіками прогресу. Елементи пошуку, фільтрації та сортування видимі одночасно для ефективного аналізу того, які вправи потребують більшої уваги без перемикання між видами.*
...
#### Огляд дашборду
![Огляд дашборду](./screenshots/08-desktop-dashboard.png)
...
#### Історія тренувань
![Історія тренувань](./screenshots/09-desktop-workout-history.png)
...
![Деталі вправи](./screenshots/10-desktop-exercise-details.png)
...
![Графіки аналітики](./screenshots/11-desktop-analytics-charts.png)
*Аналітика тривалості тренувань з візуалізацією точкової діаграми, що показує тренди тривалості тренувань в часі. Кожна точка представляє тренування, кольорово-кодована за обсягом тренування (низький/середній/високий), з динамічним розміром точок та лінією тренду для аналізу патернів. Включає картки статистики, що показують середню, найкоротшу та найдовшу тривалість тренувань.*

#### Сторінка налаштувань
![Сторінка налаштувань](./screenshots/12-desktop-settings.png)
...
### Автоматичний CI/CD з GitHub Actions

Цей проєкт включає повністю налаштований **CI/CD пайплайн**, який автоматично деплоїть на Firebase Hosting при кожному push в `main`. Також створює preview деплої для Pull Request'ів.

#### Швидкий старт (3 кроки)

1. **Отримайте Firebase Service Account токен:**
   ```bash
   firebase login:ci
   ```
   Скопіюйте токен який з'явиться (виглядає як `1//0eXXXXX...`)

2. **Додайте GitHub Secrets:**
   - Перейдіть: `GitHub Repository → Settings → Secrets and variables → Actions`
   - Додайте 7 repository secrets (використайте helper: `bash .github/copy-secrets.sh`)
   - Необхідні секрети:
     - `FIREBASE_SERVICE_ACCOUNT` (токен з кроку 1 в JSON форматі: `{"token": "ВАШ_ТОКЕН"}`)
     - `VITE_FIREBASE_API_KEY` (з `.env.local`)
...
3. **Push в GitHub:**
   ...
   - Запустить тести (`npm run test:run`)
   - Зробить production збірку (`npm run build`)
   - Задеплоїть на Firebase Hosting
   - Займе 2-4 хвилини
...
- Тести запускаються перед деплоєм (деплой зупиняється якщо тести не пройшли)
- Історія деплоїв в GitHub Actions
- Email сповіщення про успіх/невдачу деплою
- Можливість rollback через Firebase Console

#### Детальна документація

Для повної інструкції з налаштування, troubleshooting та додаткових можливостей:
- [Документація CI/CD](.github/DEPLOYMENT.md) - Детальна інструкція українською

### Варіанти ручного деплою

Якщо ви віддаєте перевагу ручному деплою або іншій платформі:

#### Варіант 1: Firebase Hosting (вручну)
```bash
...
4. Додайте змінні середовища в панелі Vercel
...
### Локальна збірка для продакшену

```bash
# Створити оптимізовану production збірку
npm run build
