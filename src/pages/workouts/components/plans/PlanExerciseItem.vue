...
const setsError = computed(() => {
  ...
  if (suggestedSets.value < CONFIG.plans.SUGGESTED_SETS_MIN || suggestedSets.value > CONFIG.plans.SUGGESTED_SETS_MAX) {
...
const weightError = computed(() => {
  ...
  if (suggestedWeight.value < CONFIG.workout.MIN_WEIGHT || suggestedWeight.value > CONFIG.workout.MAX_WEIGHT) {
...
<template>
  <Card class="p-4">
    <div class="flex gap-3">
      <!-- Drag Handle -->
      <div v-if="showDragHandle && !readonly" class="flex items-start pt-1 cursor-grab active:cursor-grabbing">
      ...
      <div class="flex-1 space-y-3">
        ...
        <div v-else-if="exercise.suggestedSets || exercise.suggestedWeight || exercise.suggestedReps" class="flex flex-wrap gap-2 text-sm">
          ...
          <span v-if="exercise.suggestedWeight" class="text-muted-foreground">
            {{ t('plans.exercise.suggestedWeight') }}: <strong>{{ exercise.suggestedWeight }} {{ unitLabel }}</strong>
          </span>
