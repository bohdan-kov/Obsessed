...
const passwordStrength = computed(() => {
  ...
  if (length < 6) return { label: 'Too short', color: 'text-destructive' }
  if (length < 8) return { label: 'Weak', color: 'text-orange-500' }
  if (
    length >= 8 &&
    hasUpperCase &&
    hasLowerCase &&
...
async function handleRegister() {
  ...
  try {
    ...
    localError.value = error.message || 'Failed to create account'
  } finally {
...
async function handleGoogleSignup() {
  ...
  try {
    ...
    localError.value = error.message || 'Failed to sign up with Google'
  } finally {
    isSubmitting.value = false
  }
}
  ...
  <div class="flex min-h-screen items-center justify-center bg-background p-4">
    <Card class="w-full max-w-md">
      <CardHeader class="space-y-1">
        ...
        <CardDescription>
          Join Obsessed to start tracking your fitness journey
        </CardDescription>
      </CardHeader>

      <CardContent class="space-y-4">
        <Alert v-if="showSuccess" variant="default" class="border-green-500">
        ...
        <form @submit.prevent="handleRegister" class="space-y-4">
          ...
          <div class="space-y-2">
            ...
            <p
              ...
              :class="['text-xs', passwordStrength.color]"
            >
